...
# 🔄 NOUVEAU : Instructions pour mise à jour de l'URL
if "ton-username" in API_URL:
    st.error("""
    ⚠️ **IMPORTANT : URL API à mettre à jour !**
    
    Dans le fichier `streamlit_app.py`, ligne 15, remplacez :
    ```python
    API_URL = "https://ton-username-getaround-api.hf.space/predict"
    ```
    
    Par votre vraie URL HF :
    ```python
    API_URL = "https://votre-nom-utilisateur-getaround-api.hf.space/predict"
    ```
    """)